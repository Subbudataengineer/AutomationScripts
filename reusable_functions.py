...
def writeRDBMSData(df,propfile,db,tbl,mode):
    config = ConfigParser()#sparkSession
    config.read(propfile)#spark.read.csv
    driver=config.get("DEVDBCRED", 'driver')#'com.mysql.cj.jdbc.Driver'
    host=config.get("DEVDBCRED", 'host')#select host
    port=config.get("DEVDBCRED", 'port')
    user=config.get("DEVDBCRED", 'user')
    passwd=config.get("DEVDBCRED", 'pass')
    url=host+":"+port+"/"+db
