...
###################################################################################################################
#Question1: I want to write DF DSL coding to SQL query ?
#create view for df and sql
...
#Question5: Can we write SQL query directly on the optimized files?
...
print("all one of the column in the given view has null will be dropped")
...
print("any of the column in the given view has null will be dropped")
...
null_any_cols_subset = spark.sql("SELECT * FROM dropped_col_df WHERE cid IS NOT NULL AND prof IS NOT NULL")
...
null_all_cols_subset = spark.sql("SELECT * FROM dropped_col_df WHERE cid IS NOT NULL OR prof IS NOT NULL")
...
#DSL Wins here since it is readily available, if we need customization we can mix with both (DSL+SQL)
...
#DSL wins here - Since unionByName function is not available in SQL
...
spark.sql("""
    CREATE OR REPLACE TEMP VIEW df_column_dedup AS
    SELECT * FROM df_row_dedup
    WHERE cid IN (
        SELECT cid FROM (
            SELECT cid, ROW_NUMBER() OVER (PARTITION BY cid ORDER BY cid) as row_num
            FROM df_row_dedup
        ) tmp
        WHERE row_num = 1
    )
""")
print("Column Level Deduplication:",spark.sql("select * from df_column_dedup").count())
...
""")
print("Drop Rows Where All Values Are Null:",spark.sql("select * from df_all_col_null").count())
...
#Drop Rows Where All Values in fname and prof Are Null:
spark.sql("""
    CREATE OR REPLACE TEMP VIEW df_all_col_null AS
    SELECT * FROM df_raw
...
df_munged_enriched_customized=spark.sql("""select *,udf_sql_agecat(coalesce(cast(age as int),0)) agecategory 
                                           from df_final""")
...
# In SQL we can use having clause, In DSL we
...
SELECT COLLECT_SET(age) AS age_freq
from (
select count(1) cnt,age from df_munged_enriched_customized
group by age
